...
@section('title', 'Edit Layanan - BersihQ Laundry')
...
@section('content')
    <div class="max-w-3xl mx-auto space-y-4 pb-20">
        ...
        <nav class="bg-white shadow-sm rounded-2xl p-4 flex items-center space-x-3 text-base">
            ...
            <span class="text-green-600 font-semibold">Edit Layanan</span>
        </nav>

        <!-- Form Container -->
        <div class="bg-white rounded-2xl shadow-sm">
            <div class="p-6">
                <form action="{{ route('services.update', $service) }}" method="POST" id="editForm">
                    @csrf
                    @method('PUT')
                    <div class="space-y-6">
                        <!-- Nama Layanan -->
                        ...
                        <div>
                            ...
                            <div class="relative">
                                ...
                                <input type="number" name="harga" id="harga"
                                    ...
                                    value="{{ old('harga', $service->harga) }}" placeholder="0" min="0" required>
                            </div>
                            @error('harga')
                                <p class="mt-1 text-sm text-red-500">{{ $message }}</p>
                            @enderror
                        ...
                        <div>
                            ...
                            <textarea name="deskripsi" id="deskripsi" rows="3"
                                ...
                                placeholder="Masukkan deskripsi layanan">{{ old('deskripsi', $service->deskripsi) }}</textarea>
                            @error('deskripsi')
                                <p class="mt-1 text-sm text-red-500">{{ $message }}</p>
                            @enderror
                        </div>
